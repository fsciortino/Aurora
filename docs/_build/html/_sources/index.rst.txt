...
The library in :py:meth:`~aurora.atomic` provides functions to load and interpolate atomic rates from ADAS ADF-11 files, as well as from ADF-15 photon emissivity coefficients (PEC) files. PEC data can alternatively be computed using the collisional-radiative model of ColRadPy, using methods in :py:func:`~aurora.radiation`.
...
Demos/Examples
--------------
A number of demonstrations and examples are available in the :py:mod:`aurora` "examples" directory. These show how to load a default namelist, modify it with specific kinetic profiles, load a magnetic equilibrium, set up an :py:mod:`aurora` object and run. A number of plotting tools and postprocessing scripts to calculate radiation are also presented. See the Tutorial page in this documentation for more information. 
