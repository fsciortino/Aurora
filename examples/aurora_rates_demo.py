...
rates_label='scd'  # chosen ADF11 file to load (SCD: ionization; ACD: recombination; CCD: CX)

# first, find name of a default file for the chosen species and process
filename = aurora.adas_files_dict()[ion][rates_label] # default SCD file for chosen species
...
atom_data = aurora.get_atom_data(ion, files=['acd', # recom
                                             'scd', # ioniz
                                             'ccd' # CX
])
...
dat_2d = aurora.interp_atom_prof(atom_data[rates_label],
                                 np.log10(ne_cm3_2d), np.log10(Te_eV_2d), # given as log10 of values
                                 log_val=False, # False --> give real units
                                 x_multiply=True) # True --> gives units of s^-1


fig,axs = plt.subplots(1,3, figsize=(13,5))
cntr0 = axs[0].contourf(R,Z,ne_cm3_2d)
cbar0 = plt.colorbar(cntr0, ax=axs[0]); cbar0.set_label(r'$n_e$ [cm${^-3}$]')
cntr1 = axs[1].contourf(R,Z,Te_eV_2d)
cbar1 = plt.colorbar(cntr1, ax=axs[1]); cbar1.set_label(r'$T_e$ [eV]')
cntr2 = axs[2].contourf(R,Z,dat_2d[:,0,:])
cbar2 = plt.colorbar(cntr2, ax=axs[2]); cbar2.set_label(rates_label.upper()+', Z=0')
plt.tight_layout()
...
_Te, fz = aurora.atomic.get_frac_abundances(
    atom_data, ne_cm3_2d, Te_eV_2d, plot=False
)
...
cbar = plt.colorbar(cntr, ax=axs[0]); cbar.set_label(fr'$n_{{{ion}17+}}$')
cntr = axs[1].contourf(R,Z,rad['line_rad'][:,16,:])
cbar = plt.colorbar(cntr, ax=axs[1]); cbar.set_label(fr'$P_{{line,{ion}17+}}$ [MW/m$^3$]')
