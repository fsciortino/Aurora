...
gradTi     = (T_core - T_edge)*T_alpha2*(1 - rhop**T_alpha1)**(T_alpha2-1)*\
             (-T_alpha1*rhop**(T_alpha1-1))*drhopdx
gradNi     = (n_core - n_edge)*n_alpha2*(1 - rhop**n_alpha1)**(n_alpha2-1)*\
             (-n_alpha1*rhop**(n_alpha1-1))*drhopdx
gradTi[-1] = gradTi[-2]
...
# begin simulation from a pre-existing profile
...
if plot: # display neoclassical transport coefficients
    ...
    plt.xlabel(r'$\rho_p$'); plt.ylabel(r'D [m$^2$/s]')
    plt.figure()
    for i, zi in enumerate(range(asim.Z_imp + 1)):
        plt.plot(asim.rhop_grid, (V_z-Vz_an)[:,0,i]/100)
    plt.xlabel(r'$\rho_p$'); plt.ylabel(r'v [m/s]')        
