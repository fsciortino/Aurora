...
aurora_dict_Ca = pylib.utils.aurora_setup(namelist, geqdsk=geqdsk)
...
logTe, fz_Ca = pylib.atomic.get_frac_abundances(atom_data, ne_avg*1e6, Te_avg, rho=rhop)

imp = namelist['imp'] = 'Ar' 
namelist['Z_imp'] = 18. 
namelist['imp_a'] = 39.948 
aurora_dict_Ar = pylib.utils.aurora_setup(namelist, geqdsk=geqdsk)
...
# get_frac_abundances takes inputs in m^-3 and eV
logTe, fz_Ar = pylib.atomic.get_frac_abundances(atom_data, ne_avg*1e6, Te_avg, rho=rhop)
...
_rV = pylib.coords.rad_coord_transform(rhop, 'rhop','r_V', geqdsk)*1e2 # m --> cm (on kin profs grid)
...
nz_tot_volavg = pylib.coords.vol_average(nz_tot[:indLCFS], aurora_dict_Ca['rhop_grid'][:indLCFS], geqdsk=geqdsk)[-1]
...
out = pylib.utils.run_aurora(aurora_dict_Ca, times_DV, D_z, V_z) #, nz_init=nz_init.T)
...
res_Ca['rad'] = pylib.radiation.compute_rad('Ca', res_Ca['rhop'], res_Ca['time'], res_Ca['nz'], 
...
out = pylib.utils.run_aurora(aurora_dict_Ar, times_DV, D_z, V_z) #, nz_init=nz_init.T)
nz, N_wall, N_div, N_pump, N_ret, N_tsu, N_dsu, N_dsul, rcld_rate, rclw_rate = out
...
res_Ar['rad'] = pylib.radiation.compute_rad('Ar', res_Ar['rhop'], res_Ar['time'], res_Ar['nz'], 
...
pylib.plot_tools.slider_plot(res_Ar['rV'], res_Ar['time'], res_Ar['nz'].transpose(1,2,0), xlabel=r'$r_V$ [cm]', ylabel='time [s]', zlabel='nz [A.U.]', labels=[fr'Ar$^{{{i}}}$$^+$' for i in np.arange(0,res_Ar['nz'].shape[1])], plot_sum=True, x_line=aurora_dict_Ca['rvol_lcfs'])
pylib.plot_tools.slider_plot(res_Ca['rV'], res_Ca['time'], res_Ca['nz'].transpose(1,2,0), xlabel=r'$r_V$ [cm]', ylabel='time [s]', zlabel='nz [A.U.]', labels=[fr'Ca$^{{{i}}}$$^+$' for i in np.arange(0,res_Ca['nz'].shape[1])], plot_sum=True)


# plot radiation profiles over radius and time
pylib.plot_tools.slider_plot(res_Ca['rV'], res_Ca['time'], res_Ca['rad']['impurity_radiation'].transpose(1,2,0)[:nz.shape[1],:,:], xlabel=r'$r_V$ [cm]', ylabel='time [s]', zlabel='Total radiation [A.U.]', labels=['Ca'+str(i) for i in np.arange(0,nz.shape[1])], plot_sum=True, x_line=aurora_dict_Ca['rvol_lcfs'])


# Peaking factor
nz_Ca_tot = np.sum(res_Ca['nz'][-1],axis=0)
indLCFS = np.argmin(np.abs(aurora_dict_Ca['rhop_grid'] - 1.0))
nz_Ca_tot_volavg = pylib.coords.vol_average(nz_Ca_tot[:indLCFS], aurora_dict_Ca['rhop_grid'][:indLCFS], geqdsk=geqdsk)[-1]
Psi_n = pylib.coords.rad_coord_transform(rhop, 'rhop','psin', geqdsk)
ind_psin02 = np.argmin(np.abs(Psi_n - 0.2))
